...
def crear_lote():
    ...
    try:
        ...
        error_response = {"success": False, "error": "Error interno del servidor"}
...
def actualizar_lote_parcial(id_lote):
    ...
    try:
        ...
        campos_presentes = [
            campo for campo in campos_permitidos if campo in request.json
        ]
...
def obtener_stock_consolidado():
    ...
    try:
        filtros = {
            ...
            "es_critico": request.args.get("es_critico", "").lower() == "true"
            if request.args.get("es_critico")
            else None,
        }
