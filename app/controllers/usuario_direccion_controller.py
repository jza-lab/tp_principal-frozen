...
class GeorefController(BaseController):
    ...
    def normalizar_direccion(self, direccion, localidad, provincia):
        ...
        try:
            ...
            if data['cantidad'] == 0:
                return self.handle_response(success=False, message="No se pudo normalizar la dirección. Verifique los datos ingresados.")

            # Tomar el primer resultado que es el más preciso
            direccion_normalizada = data['direcciones'][0]

            return self.handle_response(success=True, data=direccion_normalizada)

        except requests.exceptions.RequestException as e:
            # Manejar errores de conexión, timeout, etc.
            return self.handle_response(success=False, message=f"Error al conectar con la API de GEOREF: {e}")
        except Exception as e:
            # Manejar otros errores inesperados
            return self.handle_response(success=False, message=f"Ocurrió un error inesperado: {e}")